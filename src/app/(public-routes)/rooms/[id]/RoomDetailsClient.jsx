...
const RoomDetailsClient = ({ room }) => {
  ...
  const handleBookingSubmit = async (e) => {
    ...
    if (requestedSeats > room.capacity) {
      toast.error(`Cannot reserve more than the maximum ${room.capacity} available seats.`);
      return;
    }

    const calculatedTotal = calculateTotalPrice();
    if (calculatedTotal <= 0) {
      toast.error("Invalid timeframe choice. End time must be ahead of start time.");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`Reservation for "${room.roomName}" successfully authorized!`);
        ...
        toast.error(err.message || "Slot allocation collision or error detected.");
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="min-h-screen bg-base-100 py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-3">
              <h3 className="text-xl font-bold text-base-content tracking-tight">About This Workspace</h3>
              <p className="text-base-content/75 text-base leading-relaxed whitespace-pre-line">{room.description}</p>
            </div>

            <div className="space-y-4 pt-4">
              <h3 className="text-xl font-bold text-base-content tracking-tight">Included Amenities</h3>
            ...
            <div className="p-4 rounded-xl border border-info/20 bg-info/5 flex gap-3 text-sm text-info-content/90 leading-relaxed">
              ...
              <div>
                <strong className="font-semibold block mb-0.5">Booking Etiquette Notice:</strong>
                Please maintain appropriate noise thresholds when inside this pod resource. Ensure layout configurations are returned to native templates post-checkout.
              </div>
            </div>
          </div>
            ...
            <div className="card bg-base-100 border border-base-200 shadow-xl rounded-2xl overflow-hidden">
              <div className="card-body p-6 sm:p-8 space-y-6">
                <div className="flex items-baseline justify-between border-b border-base-200 pb-4">
                  <span className="text-sm font-semibold text-base-content/60">Resource Rate</span>
                  <div className="flex items-baseline text-base-content">
                    <FiDollarSign className="w-4 h-4 text-success self-center font-bold" />
                    <span className="text-3xl font-extrabold tracking-tight">{room.pricePerHour.toFixed(2)}</span>
                    <span className="text-sm font-medium text-base-content/50 ml-1">/ hour</span>
                  </div>
                </div>

                <div className="space-y-3.5 text-sm font-medium text-base-content/80">
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-base-content/60"><FiClock className="w-4 h-4" /> Min Duration</span>
                    <span>1.0 Hour</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-base-content/60"><FiUsers className="w-4 h-4" /> Allocation Rule</span>
                    <span>Entire Room</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="flex items-center gap-2 text-base-content/60"><FiCalendar className="w-4 h-4" /> Instant Approval</span>
                ...
                <div className="pt-2">
                  <button
                    onClick={() => {
                      if (!session) {
                        toast.error("Please log in to register a workspace reservation.");
                        return;
                      }
                      setIsModalOpen(true);
                    }}
      ...
      {isModalOpen && (
        <dialog open className="modal modal-bottom sm:modal-middle bg-black/50 backdrop-blur-xs z-50 animate-fade-in">
          <div className="modal-box bg-base-100 border border-base-300 max-w-md p-6 relative rounded-2xl shadow-2xl">
            ...
            <p className="text-xs text-base-content/60 mb-6">
              Hourly cost configuration: <span className="font-semibold text-neutral">${room.pricePerHour.toFixed(2)}/hr</span>
            </p>

            <form onSubmit={handleBookingSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4 bg-base-200/50 p-3 rounded-xl border border-base-200">
                <div className="flex flex-col">
                  <span className="text-xs font-semibold text-base-content/50 uppercase tracking-wider">Seats Available</span>
                  <span className="text-lg font-extrabold text-neutral">{room.capacity} Total</span>
                </div>
              ...
              {calculateTotalPrice() > 0 && (
                <div className="p-4 rounded-xl bg-base-200/60 border border-base-300 flex justify-between items-center mt-2 animate-fade-in">
                  <span className="text-xs font-bold uppercase tracking-wider text-base-content/60">Estimated Cost</span>
                  <span className="text-xl font-black text-neutral">${calculateTotalPrice().toFixed(2)}</span>
                </div>
              )}
              ...
              <div className="modal-action pt-2">
                <button
                  type="submit"
                  disabled={isSubmitting || calculateTotalPrice() <= 0 || requestedSeats > room.capacity || requestedSeats < 1}
                  className="btn btn-neutral w-full normal-case font-bold shadow-md disabled:bg-base-300"
                >
                  {isSubmitting ? "Processing Schedule..." : "Confirm Booking Allocation"}
                </button>
              </div>
            </form>
          </div>
